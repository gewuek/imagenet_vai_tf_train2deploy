...
#define KRENEL_CONV "inceptionv3_0"

#define TASK_CONV_INPUT "conv2d_Conv2D"
#define TASK_CONV_OUTPUT "predictions_MatMul"
  ...
  for(int i = 0; i < 3; ++i)
    for(int j = 0; j < image.rows; ++j)
      for(int k = 0; k < image.cols; ++k) {
		    data[j*image.rows*3+k*3+i] = (image.at<Vec3b>(j,k)[i] - (int8_t)mean[i]) * scale;// Jason
      }
...
inline void set_input_image(DPUTask *task, const string& input_node, const cv::Mat& image, float* mean){
  ...
  // Size size_224_224(224,224);
  Size size_299_299(299,299);
  // resize(image, cropped_img,size_224_224);// Jason
  resize(image, cropped_img,size_299_299);// Jason
...
void runResnet(DPUTask *taskConv) {
  ...
  for (auto &image_name : images) {
    ...
    float mean[3] = {103.939, 116.779, 123.68}; // Jason
